-- Script simple para actualizar la tabla historial_clics para evitar duplicados
...
-- Usar una subconsulta para obtener el metadata del registro más reciente
WITH clics_consolidados AS (
    SELECT 
        ...
        -- Obtener el metadata del registro más reciente usando ROW_NUMBER
        FIRST_VALUE(metadata) OVER (
            PARTITION BY usuario_id, producto_id, tipo_clic, origen_clic 
            ORDER BY fecha_clic DESC
        ) as metadata
    FROM historial_clics
    GROUP BY usuario_id, producto_id, tipo_clic, origen_clic, metadata
)
